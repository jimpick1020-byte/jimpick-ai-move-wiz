        ...
        {cells.map((cell, index) => {
          ...
          return (
            <button
              ...
              className="relative flex min-h-[53px] min-w-0 flex-col items-center pt-1"
            >
              <span className={`flex h-7 min-w-7 items-center justify-center rounded-full px-1 text-[15px] font-bold tabular-nums ${selected ? "bg-[#1671E8] text-white" : !cell.current ? "text-[#C4CAD4]" : dow === 0 ? "text-[#FF4D4F]" : dow === 6 ? "text-[#1671E8]" : "text-[#111827]"} ${isToday && !selected ? "ring-1 ring-[#1671E8]" : ""}`}>
                {cell.day}
              </span>
              <span className="mt-0.5 flex h-3 items-center justify-center gap-1">
                {progressing > 0 && <i className="h-1.5 w-1.5 rounded-full bg-[#1671E8]" />}
                {completed > 0 && <i className="h-1.5 w-1.5 rounded-full bg-[#4ED2BC]" />}
                {dayBookings.length > 1 && <b className="text-[9px] leading-none text-[#667085]">{dayBookings.length}건</b>}
              </span>
              {cell.son && <span className="absolute bottom-0 rounded-full bg-[#FF6B68] px-1.5 py-0.5 text-[8px] font-black leading-none text-white">손</span>}
            </button>
          );
        })}
      </div>
