...
export const Route = createFileRoute("/share/$id")({
  ...
  loader: async ({ params, deps }) => {
    if (deps.staff || !deps.t || deps.t.length < 8 || deps.t.length > 80 || params.id.length > 120) {
      return { valid: false as const, card: "quote" as CardType };
    }
    ...
    try {
      ...
      return { valid: result.valid, card };
    } catch {
      return { valid: false as const, card };
    }
  },
  head: ({ loaderData, params, match }) => {
    ...
    const href = valid && origin ? `${origin}${match.pathname}${window.location.search}` : undefined;
    const image = preview && origin ? new URL(preview.image, origin).href : undefined;
