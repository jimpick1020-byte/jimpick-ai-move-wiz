...
export function kakaoErrorMessage(status: number, body: string): string {
  ...
  if (status === 401)
    return "카카오 REST API 키가 올바르지 않습니다. (REST API 키인지 확인해 주세요)";
  if (status === 403)
    return "카카오 API 사용 권한이 없습니다. 앱 설정에서 카카오맵/로컬 서비스를 활성화해 주세요.";
  if (status === 429) return "카카오 API 호출 한도를 초과했습니다. 잠시 후 다시 시도해 주세요.";
  return "주소 검색에 실패했습니다. 잠시 후 다시 시도해 주세요.";
}

/** OpenStreetMap(Nominatim) 검색 결과 한 건 (필요한 항목만) */
interface OsmPlace {
  display_name?: string;
  name?: string;
  lon?: string;
...
export async function searchOsm(query: string): Promise<KakaoPlace[]> {
  try {
    ...
    if (!res.ok) return [];
    const list = (await res.json()) as OsmPlace[];
    return list.map((d) => {
