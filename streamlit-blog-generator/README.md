...
그래서 이 앱은 **사진을 이미지 호스팅(imgbb)에 올려 공개 URL을 받은 뒤,
블로그 본문에 `<img>` 태그로 삽입**하는 방식으로 사진을 게시글에 보이게 합니다.

1. [imgbb API](https://api.imgbb.com/)에서 무료 키를 발급받습니다.
2. `IMGBB_API_KEY` 를 `secrets.toml`(또는 환경변수/사이드바)에 넣습니다.
3. 발행 시 업로드한 매물 사진이 자동으로 글 본문 아래에 삽입됩니다.
...
- **첫 사진은 글 맨 위 대표 이미지로 고정**되고, 나머지 사진은 본문 문단 사이사이에
  고르게 분산되어 삽입됩니다. 배치 방식을 바꾸려면 `naver.build_contents_html()` 을
  수정하면 됩니다.
