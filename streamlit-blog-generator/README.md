...
- **첫 사진은 글 맨 위 대표 이미지로 고정**(전체폭·둥근 모서리·그림자로 강조)되고
  바로 아래에 **캡션**(`▲ [매물명] 대표 사진` 형식)이 붙습니다. 나머지 사진은 본문 문단
  사이사이에 고르게 분산되어 삽입됩니다. 배치·스타일·캡션을 바꾸려면
  `naver.build_contents_html()` 을 수정하면 됩니다.
